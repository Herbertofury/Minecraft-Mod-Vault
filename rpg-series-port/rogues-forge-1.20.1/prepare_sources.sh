...
CURRENT_GRADLE_BLOB="ccc8b423aa4f8a8526d79d27a2fd862da12dd8cc"
...
HIST_GRADLE_BLOB="d01d500b3b98a14b0181b93fdd6fdb68cb5be6c9"
HIST_VERSION="1.2.0"
HIST_MC="1.20.1"
...
assert_prop(){
  ...
  grep -Fqx "$key=$expected" "$file" || fail "expected $key=$expected in $file"
}

fetch_archive(){
  local commit="$1" dest="$2" label="$3"
  local archive="$TMP/$label.tar.gz" extract="$TMP/$label-extract"
  mkdir -p "$extract"
...
verify_tree_via_api(){
  local commit="$1" expected_tree="$2" label="$3"
  local json="$TMP/$label-commit.json"
  curl --fail --location --silent --show-error --retry 4 --retry-delay 2 \
...
# Historical source is intentionally never overlaid into materialized-current.
...
# Recompute the materialized manifest and require exact stability inside the same run.
