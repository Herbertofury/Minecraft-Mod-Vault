...
# the 1.20.1 packaged server can interrogate the exact live target relation. Run
# #246 proved the original single sample 0.5 seconds after spawn was itself a
# race: it read target=0, then the same control Husk later acquired and killed the
# player. Keep the direct relation test, but give the control 12 blocks of runway
# and poll the live target relation for four seconds while it is still >=5 blocks
# away. The identically separated Stealth Husk is observed over the same window;
# it must never acquire the player, must remain >=5 blocks away, and Stealth must
# still be active. This strengthens the discriminator without freezing AI,
# mounting mobs, relying on damage timing, or instrumenting product code.
...
new="""send_cmd 'tp @a 20.5 100 0.5 0 0'; sleep 2; send_cmd 'execute positioned 20.5 100 0.5 if entity @a[distance=..1.0,limit=1] run say ROGUES_CONTROL_ARENA_POSITION_READY'; wait_marker 'ROGUES_CONTROL_ARENA_POSITION_READY' 20 'control arena position'; send_cmd 'execute at @a[limit=1] run summon minecraft:husk ~12 ~ ~ {Tags:[\"rogp\",\"rogp_control_husk\"],Silent:1b,PersistenceRequired:1b}'; send_cmd 'scoreboard players set control_target rogp 0'; for _ in {1..16}; do send_cmd 'execute as @e[type=minecraft:husk,tag=rogp_control_husk,limit=1,sort=nearest] at @s on target if entity @s[type=minecraft:player,distance=5..] run scoreboard players set control_target rogp 1'; send_cmd 'execute if score control_target rogp matches 1 run say ROGUES_STEALTH_VISIBILITY_CONTROL_PASS'; sleep 0.25; done; wait_marker 'ROGUES_STEALTH_VISIBILITY_CONTROL_PASS' 20 'non-stealth hostile direct target control'; send_cmd 'kill @e[tag=rogp_control_husk]'; send_cmd 'tp @a 30.5 100 0.5 0 0'; sleep 2; send_cmd 'execute positioned 30.5 100 0.5 if entity @a[distance=..1.0,limit=1] run say ROGUES_STEALTH_ARENA_POSITION_READY'; wait_marker 'ROGUES_STEALTH_ARENA_POSITION_READY' 20 'Stealth arena position'; send_cmd 'effect give @a rogues:stealth 30 0 true'; send_cmd 'execute at @a[limit=1] run summon minecraft:husk ~12 ~ ~ {Tags:[\"rogp\",\"rogp_stealth_husk\"],Silent:1b,PersistenceRequired:1b}'; send_cmd 'scoreboard players set stealth_target rogp 0'; send_cmd 'scoreboard players set stealth_distance rogp 1'; for _ in {1..16}; do send_cmd 'execute as @e[type=minecraft:husk,tag=rogp_stealth_husk,limit=1,sort=nearest] on target if entity @s[type=minecraft:player] run scoreboard players set stealth_target rogp 1'; send_cmd 'execute as @e[type=minecraft:husk,tag=rogp_stealth_husk,limit=1,sort=nearest] at @s unless entity @a[distance=5..,limit=1] run scoreboard players set stealth_distance rogp 0'; sleep 0.25; done; send_cmd 'execute store success score stealth_present rogp run effect clear @a[limit=1] rogues:stealth'; send_cmd 'execute if score stealth_target rogp matches 0 if score stealth_distance rogp matches 1 if score stealth_present rogp matches 1 run say ROGUES_STEALTH_VISIBILITY_RUNTIME_PASS'; wait_marker 'ROGUES_STEALTH_VISIBILITY_RUNTIME_PASS' 20 'stealth direct hostile target suppression'"""
if text.count(old) != 1:
    raise SystemExit(f'[Rogues deep acceptance] expected exactly one original hostile visibility QA block, found {text.count(old)}')
path.write_text(text.replace(old,new,1))
PY
grep -Fq 'for _ in {1..16}; do send_cmd' "$PLAYER_QA" || { echo '[Rogues deep acceptance] target-acquisition polling window missing' >&2; exit 2; }
