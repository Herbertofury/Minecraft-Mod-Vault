...
# distributable sealing and runtime proof of the 1.10.3 tooltip key registration. The warning-screen
# patch changes only Forge's interactive CI presentation; genuine mod-loading errors remain fatal.
python3 - "$GRAD" <<'PY'
...
bash -n "$INNER"
python3 -m py_compile "$PATCHER" "$WARNING_PATCHER" "$NONCE_PATCHER" "$NONCE_CANON" "$SEALER"
echo '[Spell Engine 1.10.4] EXACT_CERTIFIED_NESTED_BYTES_ARCHITECTURY_NONCE_NATIVE_TOOLTIP_AND_WARNING_SAFE_RUNTIME_WRAPPER_READY'
exec bash "$INNER"
