...
) > "$PORT/more-rpg-library-replay-source-manifest.sha256"
...
def pack(root, out):
    ...
    with zipfile.ZipFile(out,'w',compression=zipfile.ZIP_DEFLATED,compresslevel=9) as z:
        for rel,p in sorted(files,key=lambda x:x[0].as_posix()):
            info=zipfile.ZipInfo(rel.as_posix(),(1980,1,1,0,0,0)); info.compress_type=zipfile.ZIP_DEFLATED; info.external_attr=(0o100644 << 16)
...
REPLAY_ZIP="$PORT/more-rpg-library-2.7.2-forge-1.20.1-source-replay-ci.zip"
