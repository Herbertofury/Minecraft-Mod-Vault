...
# before the QA-only semantic server. A later prepare() reconstructs pristine
# product sources before native-client and fresh packaged-server lanes.
python3 - "$BASE" <<'PY'
...
bash -n "$BASE"
echo '[Gazebos v4] POST_SEAL_CERTIFIED_SPA_MIXINEXTRAS_USERDEV_BRIDGE_PASS'
exec bash "$V3"
