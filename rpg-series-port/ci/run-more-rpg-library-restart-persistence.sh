...
RUN="$WORK/forge/run"
SAVE='MRPG-QA'
WORLD="$RUN/saves/$SAVE"
QA_PACK="$WORLD/datapacks/more-rpg-runtime-qa"
...
mapfile -t PLAYERDATA < <(find "$WORLD/playerdata" -maxdepth 1 -type f -name '*.dat' 2>/dev/null | sort || true)
((${#PLAYERDATA[@]} > 0)) || { echo '[More RPG 2.7.2] Stage-1 produced no persisted playerdata file' >&2; exit 1; }
BEFORE_SHA="$(sha256sum "${PLAYERDATA[0]}" | awk '{print $1}')"
printf '[More RPG 2.7.2] RESTART_PERSISTENCE_PLAYERDATA_PRESENT_PASS file=%s sha256=%s\n' "${PLAYERDATA[0]#$WORLD/}" "$BEFORE_SHA"

# Replace the first-run function: the restart gate must never grant the tag itself. It can only emit the
# marker when the tag persisted from the prior integrated-world launch.
...
MCFUNCTION
if grep -Fq 'effect give' "$QA_PACK/data/more_rpg_qa/functions/tick.mcfunction"; then
  echo '[More RPG 2.7.2] restart datapack illegally re-grants fatal poison' >&2; exit 1
fi
ARGS=(
...
LOG="$PORT/more-rpg-restart-persistence-client.log"; : > "$LOG"
ACTIVE_PID=''
...
while ((SECONDS<DEADLINE)); do
  ...
  if [[ -f "$LATEST" ]] && grep -Fq 'Reloading ResourceManager' "$LATEST" && grep -Fq 'Backend library: LWJGL' "$LATEST" && grep -Fq '[More RPG QA] PERSISTED_PLAYER_TAG_SEEN' "${FILES[@]}"; then PASS=1; break; fi
...
! grep -Eiq "$FATAL" "$LOG" "$LATEST"
mapfile -t AFTER_PLAYERDATA < <(find "$WORLD/playerdata" -maxdepth 1 -type f -name '*.dat' 2>/dev/null | sort || true)
((${#AFTER_PLAYERDATA[@]} > 0))
AFTER_SHA="$(sha256sum "${AFTER_PLAYERDATA[0]}" | awk '{print $1}')"
printf '[More RPG 2.7.2] SAME_SAVE_RESTART_PERSISTENCE_PASS marker=PERSISTED_PLAYER_TAG_SEEN playerdata_before=%s playerdata_after=%s\n' "$BEFORE_SHA" "$AFTER_SHA"
