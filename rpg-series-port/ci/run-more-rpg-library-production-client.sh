...
for f in "$OUT" "$SPELL_ENGINE_JAR" "$SPELL_POWER_JAR" "$RANGED_JAR" "$TINY_JAR" "$CLOTH_FORGE_JAR" "$PLAYER_ANIM_FORGE_JAR"; do
...
PROD="$PORT/.production-forge-client"
MCROOT="$PROD/minecraft-root"
RUN="$PROD/run"
NATIVES="$PROD/natives"
rm -rf "$PROD"
mkdir -p "$MCROOT" "$RUN/mods" "$RUN/config" "$RUN/saves" "$NATIVES"
...
FORGE_JSON="$MCROOT/versions/$VERSION_ID/$VERSION_ID.json"
test -f "$FORGE_JSON"
...
assets=root/'assets'; (assets/'indexes').mkdir(parents=True,exist_ok=True); (assets/'objects').mkdir(parents=True,exist_ok=True)
def get(url):
...
manifest_raw=get('https://piston-meta.mojang.com/mc/game/version_manifest_v2.json')
...
vanilla=root/'versions'/'1.20.1'; vanilla.mkdir(parents=True,exist_ok=True)
(vanilla/'1.20.1.json').write_bytes(version_raw)
...
index_id=ai['id']; (assets/'indexes'/f'{index_id}.json').write_bytes(index_raw)
index=json.loads(index_raw); hashes=sorted({o['hash'] for o in index['objects'].values()})
def ensure(h):
...
forge=json.loads(forge_json.read_text()); vanilla=json.loads((root/'versions'/'1.20.1'/'1.20.1.json').read_text())
libs=root/'libraries'
...
def artifact_path(lib):
    ...
    group,artifact,version=parts[:3]; classifier=parts[3] if len(parts)>3 else None
...
for doc in (vanilla,forge):
    for lib in doc.get('libraries',[]):
        ...
        if p and p.is_file() and str(p) not in seen:
            cp.append(p); seen.add(str(p))
        native_key=lib.get('natives',{}).get('linux')
        if native_key:
            native_key=native_key.replace('${arch}','64')
            cl=lib.get('downloads',{}).get('classifiers',{}).get(native_key)
...
def resolve_args(entries):
    ...
    for e in entries:
        ...
        v=e.get('value',[]); v=[v] if isinstance(v,str) else v
        out.extend(expand(x) for x in v)
    return out
jvm=resolve_args(forge.get('arguments',{}).get('jvm',[]))
jvm += [f'-Djava.library.path={natives}', '-cp', classpath]
jvm += ['-Xmx4G','-Dmixin.debug.export=true','-Dmixin.debug.export.filter=net.minecraft.client.gui.**','-Dmixin.debug.export.decompile=false','-Dmixin.debug.verbose=true']
...
cp "$OUT" "$RUN/mods/more-rpg-library-forge.jar"
cp "$SPELL_ENGINE_JAR" "$RUN/mods/spell-engine-forge.jar"
cp "$SPELL_POWER_JAR" "$RUN/mods/spell-power-forge.jar"
cp "$RANGED_JAR" "$RUN/mods/ranged-weapon-api-forge.jar"
cp "$TINY_JAR" "$RUN/mods/tiny-config-forge.jar"
cp "$CLOTH_FORGE_JAR" "$RUN/mods/cloth-config-forge.jar"
cp "$PLAYER_ANIM_FORGE_JAR" "$RUN/mods/player-animation-lib-forge.jar"
...
[[ -d "$SOURCE_WORLD" ]] || { echo '[More RPG 2.7.2] Stage-1 source world missing for production client' >&2; exit 1; }
cp -a "$SOURCE_WORLD" "$RUN/saves/MRPG-QA"
printf 'earlyWindowControl = false\n' > "$RUN/config/fml.toml"
rm -rf "$RUN/logs" "$RUN/.mixin.out"
mapfile -d '' -t JVM_ARGS < "$MCROOT/jvm.nul"
mapfile -d '' -t GAME_ARGS < "$MCROOT/game.nul"
MAIN_CLASS="$(tr -d '\r\n' < "$MCROOT/main-class.txt")"
LOG="$PORT/more-rpg-production-client.log"; : > "$LOG"
ACTIVE_PID=''
cleanup() { [[ -z "${ACTIVE_PID:-}" ]] || { kill -TERM "$ACTIVE_PID" 2>/dev/null || true; wait "$ACTIVE_PID" 2>/dev/null || true; }; }
trap cleanup EXIT INT TERM
env -u MOD_CLASSES LIBGL_ALWAYS_SOFTWARE=1 MESA_LOADER_DRIVER_OVERRIDE=llvmpipe ALSOFT_DRIVERS=null \
  xvfb-run -a -s '-screen 0 1280x720x24 +extension GLX +extension RENDER -noreset' \
  java "${JVM_ARGS[@]}" "$MAIN_CLASS" "${GAME_ARGS[@]}" </dev/null > "$LOG" 2>&1 &
...
while ((SECONDS<DEADLINE)); do
  ...
  if [[ -f "$LATEST" ]] && grep -Fq 'ModLauncher launch target: forgeclient' "$LATEST" && grep -Fq 'ModLauncher naming: srg' "$LATEST" && grep -Fq 'Reloading ResourceManager' "$LATEST" && grep -Fq 'Backend library: LWJGL' "$LATEST" && grep -Fq '[More RPG QA] FATAL_POISON_APPLIED' "${FILES[@]}"; then PASS=1; break; fi
...
grep -Eiq 'DrawHeartsMixin.*more-rpg-classes\.mixins\.json|more-rpg-classes\.mixins\.json.*DrawHeartsMixin' "$LOG" "$LATEST"
MIXIN_OUT="$RUN/.mixin.out"; [[ -d "$MIXIN_OUT" ]]
...
((${#HEART_TARGETS[@]} > 0)) || { echo '[More RPG 2.7.2] production SRG HUD transform missing HeartRegistry' >&2; exit 1; }
EVIDENCE="$PORT/production-transformed-hud-evidence"
rm -rf "$EVIDENCE"; mkdir -p "$EVIDENCE"
cp "${HEART_TARGETS[0]}" "$EVIDENCE/more-rpg-production-transformed-hud-target.class"
HUD_SHA="$(sha256sum "$EVIDENCE/more-rpg-production-transformed-hud-target.class" | awk '{print $1}')"
printf '%s  more-rpg-production-transformed-hud-target.class\n' "$HUD_SHA" > "$EVIDENCE/more-rpg-production-transformed-hud-target.sha256"
printf '%s\n' "${HEART_TARGETS[0]#$MIXIN_OUT/}" > "$EVIDENCE/more-rpg-production-transformed-hud-target.source-path.txt"
strings -a "$EVIDENCE/more-rpg-production-transformed-hud-target.class" | sort -u > "$EVIDENCE/more-rpg-production-transformed-hud-target.strings.txt"
grep -F 'net/more_rpg_classes/client/heart/HeartRegistry' "$EVIDENCE/more-rpg-production-transformed-hud-target.strings.txt" >/dev/null
kill -TERM "$PID" 2>/dev/null || true; wait "$PID" 2>/dev/null || true; ACTIVE_PID=''
printf '[More RPG 2.7.2] PRODUCTION_DRAW_HEARTS_MIXIN_TRANSFORMED_PASS targets=%s first=%s sha256=%s\n' "${#HEART_TARGETS[@]}" "${HEART_TARGETS[0]#$MIXIN_OUT/}" "$HUD_SHA"
printf '[More RPG 2.7.2] PRODUCTION_FORGE_CLIENT_SRG_PASS assets=%s mod_sha=%s launch_target=forgeclient naming=srg\n' "$ASSET_COUNT" "$(sha256sum "$OUT" | awk '{print $1}')"
