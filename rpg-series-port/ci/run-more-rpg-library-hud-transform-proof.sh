...
RUN="$WORK/forge/run"; SAVE='MRPG-QA-HUD'; rm -rf "$RUN/logs" "$RUN/.mixin.out" "$RUN/saves/$SAVE"; mkdir -p "$RUN/config" "$RUN/saves"; cp -a "$SOURCE_WORLD" "$RUN/saves/$SAVE"; printf 'earlyWindowControl = false\n' > "$RUN/config/fml.toml"
LOG="$PORT/more-rpg-hud-transform-client.log"; : > "$LOG"; ACTIVE_PID=''
...
env LIBGL_ALWAYS_SOFTWARE=1 MESA_LOADER_DRIVER_OVERRIDE=llvmpipe ALSOFT_DRIVERS=null JAVA_TOOL_OPTIONS="$MIXIN_OPTS ${JAVA_TOOL_OPTIONS:-}" xvfb-run -a -s '-screen 0 1280x720x24 +extension GLX +extension RENDER -noreset' gradle --no-daemon -p "$WORK" :forge:runClient --args="--width 1280 --height 720 --quickPlaySingleplayer $SAVE" "${ARGS[@]}" </dev/null > "$LOG" 2>&1 &
...
while ((SECONDS<DEADLINE)); do LATEST="$RUN/logs/latest.log"; FILES=("$LOG"); [[ -f "$LATEST" ]] && FILES+=("$LATEST"); if grep -Eiq "$FATAL" "${FILES[@]}"; then tail -n 700 "${FILES[@]}" 2>/dev/null || true; exit 1; fi; if [[ -f "$LATEST" ]] && grep -Fq 'Reloading ResourceManager' "$LATEST" && grep -Fq 'Backend library: LWJGL' "$LATEST" && grep -Fq '[More RPG QA] FATAL_POISON_APPLIED' "${FILES[@]}"; then READY=1; break; fi; if ! kill -0 "$PID" 2>/dev/null; then wait "$PID" || true; ACTIVE_PID=''; tail -n 700 "${FILES[@]}" 2>/dev/null || true; exit 1; fi; sleep 1; done
[[ "$READY" -eq 1 ]] || { tail -n 700 "$LOG" "$RUN/logs/latest.log" 2>/dev/null || true; exit 1; }
LATEST="$RUN/logs/latest.log"; grep -Eiq 'DrawHeartsMixin.*more-rpg-classes\.mixins\.json|more-rpg-classes\.mixins\.json.*DrawHeartsMixin' "$LOG" "$LATEST"
MIXIN_OUT="$RUN/.mixin.out"; [[ -d "$MIXIN_OUT" ]]; mapfile -t TARGETS < <(grep -aRl 'net/more_rpg_classes/client/heart/HeartRegistry' "$MIXIN_OUT" --include='*.class' 2>/dev/null || true); ((${#TARGETS[@]} > 0)) || { echo '[More RPG 2.7.2] no transformed GUI target contains More RPG HeartRegistry bytecode' >&2; exit 1; }
printf '[More RPG 2.7.2] DRAW_HEARTS_MIXIN_TRANSFORMED_CLIENT_PASS targets=%s first=%s\n' "${#TARGETS[@]}" "${TARGETS[0]#$MIXIN_OUT/}"; stop_tree "$PID"; ACTIVE_PID=''; echo '[More RPG 2.7.2] MAPPED_CLIENT_FATAL_POISON_HUD_PROOF_PASS effect=more_rpg_classes:fatal_poison transformed_hud=true'
