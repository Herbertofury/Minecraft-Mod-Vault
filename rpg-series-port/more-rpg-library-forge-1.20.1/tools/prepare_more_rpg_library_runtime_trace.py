...
enchantment_wave7 = Path(__file__).with_name('prepare_more_rpg_library_1201_enchantment_wave7.py')
client_lifecycle = Path(__file__).with_name('prepare_more_rpg_library_1201_client_lifecycle.py')
...
for tool in (forge_registry_wave6, enchantment_wave7, client_lifecycle, draw_hearts_signature, stealth_model_render, external_school_attributes, deferral):
    if not tool.is_file():
...
subprocess.run([sys.executable, str(enchantment_wave7), str(root)], check=True)
subprocess.run([sys.executable, str(client_lifecycle), str(root)], check=True)
...
print('[More RPG 2.7.2] ENCHANTMENT_DATA_1201_RUNTIME_BRIDGE_READY source=upstream-2.7.2-data-enchantments')
print('[More RPG 2.7.2] FORGE_CLIENT_LIFECYCLE_1201_RUNTIME_BRIDGE_READY source=upstream-2.7.2-neoforge-client-lifecycle')
