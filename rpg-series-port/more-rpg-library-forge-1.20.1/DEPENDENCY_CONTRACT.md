...
Heavy More RPG graduation MUST NOT start until Spell Engine **1.10.4** has a frozen Forge 1.20.1 release identity and an independent frozen replay. The user's supplied Spell Engine 1.10.2 build remains the historical minimum floor, but 1.10.4 is the final foundation authority because its tooltip-details binding, fallback loot injection, loot-entry access crash fix, and externally configurable Critical Strike compatibility are required parity. Spell Engine and Spell Power are public hard ecosystem requirements of the modern library, so using an unfinished Spell Engine port would make every downstream class/content result untrustworthy.
...
- Gazebos 2.2.0 Forge 1.20.1 (graduated and canonically promoted)
- Spell Engine 1.10.4 Forge 1.20.1 only after its unresolved graduation identity placeholder is replaced by a frozen/replayed JAR SHA
...
- Spell Engine: hard ordering/runtime foundation; freeze 1.10.4 identity before More RPG.
...
- Critical Strike: compile/optional integration in modern common code; absence must not prevent base library startup unless source semantics prove otherwise. When present, bind through the graduated Spell Engine 1.10.4 compatibility seam rather than introducing a hard circular dependency.
...
3. certified foundation identity checks before build, including frozen Spell Engine 1.10.4;
